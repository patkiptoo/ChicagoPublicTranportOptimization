...
#  Define a Faust Stream that ingests data from the Kafka 
#  Connect stations topic and places it into a new topic 
...
# Define the output Kafka Topic (transformed)
...
# Note that "line" is the color of the station. So if the `Station` record has 
# the field `red` set to true, then you would set the `line` of the `TransformedStation` 
# record to the string `"red"`
@app.agent(topic)
