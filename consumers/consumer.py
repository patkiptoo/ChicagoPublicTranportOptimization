...
class KafkaConsumer:
    ...
    def on_assign(self, consumer, partitions):
        ...
        # set the partition offset to
        # the beginning or earliest
        for partition in partitions:
            partition.offset = OFFSET_BEGINNING
            logger.info("partition offset for topic %s has been set to beginning.", self.topic_name_pattern)
    ...
    def _consume(self):
        ...
        if message is None:
            logger.info("no message received from topic by consumer")
            return 0
            ...
            logger.info(f"consumer recieved message with key {message.key()} value {message.value()}")
            return 1

    def close(self):
        """Cleans up any open kafka consumers"""
        logger.info("closing consumers {}".format(self.topic_name_pattern))
        self.consumer.close()