...
class Producer:
    ...
    def topic_exists(self, client, topic_name):
        ...
        topic_list = client.list_topics(timeout=7)
        return topic_name in set(t.topic for t in iter(topic_list.topics.values()))
        ...
        if self.topic_exists(client, self.topic_name):
            ...
            for _, future in futures.items():
                try:
                    ...
                    logger.error(f"Exception as topic creation %s", self.topic_name)
    ...
    def close(self):
        ...
        self.producer.flush()
        logger.info(f"Producer close complete")
